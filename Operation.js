exports.calculation = (firstNumber, operator, secondNumber) => {
  ...
  switch (operator) {
    case "+":
    case "-":
      ({ resultNumerator, resultDenominator } = addOrSubstract(
        firstNumber,
        operator,
        secondNumber
      ));
    ...
    case "/":
    case "*":
      ({ resultNumerator, resultDenominator } = multiplyOrDivide(
        firstNumber,
        operator,
        secondNumber
      ));
  ...
  ({ resultNumerator, resultDenominator, resultWholeNumber, flagWhole } =
    simplify(resultNumerator, resultDenominator));

  //throwing error if calculations cant be made with the input.
  if (
    isNaN(resultWholeNumber) ||
    isNaN(resultNumerator) ||
